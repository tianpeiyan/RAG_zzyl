...
public class ReservationServiceImpl implements ReservationService {
    ...
    public void add(ReservationDto dto) {
        ...
        if (StrUtil.isBlank(dto.getName())) {
            throw new BaseException(NAME_CANNOT_NULL);
        }
        if (StrUtil.isBlank(dto.getMobile())) {
            throw new BaseException(PHONE_CANNOT_NULL);
        }
        // if (StrUtil.isBlank(dto.getIdCard())) {
        //    throw new BaseException("身份证号不能为空");
        // }
        if (dto.getType() == null) {
            throw new BaseException(RESERVATION_TYPE_CANNOT_NULL);
        }
        if (dto.getTime() == null) {
            throw new BaseException(RESERVATION_TIME_CANNOT_NULL);
        }
        if (dto.getType() == 1 && dto.getElderId() == null) {
            throw new BaseException(TYPE_1);
        }

        // 冲突检测：检查同一手机号在同一时间段是否有待报到的预约
        List<Reservation> conflicts = reservationMapper.findAll(null, dto.getMobile(), dto.getTime(), dto.getTime());
        for (Reservation r : conflicts) {
            if (r.getStatus().equals(ReservationStatus.PENDING.getOrdinal())) {
                throw new BaseException(REPEAT);
            }
        }
        ...
        } else if (mgtUserId != null) {
            reservation.setCreateBy(mgtUserId);
        }
    ...
    public void update(Long id, ReservationDto dto) {
        ...
        if (reservation != null) {
            BeanUtils.copyProperties(dto, reservation, "id", "createBy", "createTime");
     ...
     */
    @Override
    public PageResponse<ReservationVo> findByPage(int page, int size, String name, String phone, Integer status, Integer type, LocalDateTime startTime, LocalDateTime endTime, Long createBy) {
        PageHelper.startPage(page, size);
        Page<Reservation> byPage = reservationMapper.findByPage(page, size, name, phone, status, type, createBy, startTime, endTime);
